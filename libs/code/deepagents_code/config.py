...
|____/|____/|_|\\_\\
  ...
  ____ ___  ____ ___ _     ___ _____
 / ___/ _ \\|  _ \\_ _| |   / _ \\_   _|
| |  | | | | |_) | || |  | | | || |
| |__| |_| |  __/| || |__| |_| || |
 \\____\\___/|_|  |___|_____\\___/ |_|
                      v{__version__}
"""
...
class Settings:
    ...
    @property
    def manifest_db_path(self) -> str | None:
        """Path to manifest.db: DEEPAGENTS_MANIFEST_DB override, else bundled."""
        import os

        raw = os.environ.get("DEEPAGENTS_MANIFEST_DB")
        if raw:
            return raw
        bundled = Path(__file__).parent / "data" / "sdkm" / "manifest.db"
        return str(bundled) if bundled.exists() else None

    @property
    def has_manifest_db(self) -> bool:
        """Whether an SDK Manager manifest.db is available (env override or bundled)."""
        path = self.manifest_db_path
